...
def analyze_packet_data(df):
    ...
    ip_communication_protocols["Percentage"] = ip_communication_protocols.groupby(["Source IP", "Destination IP"])[
...
def print_results(total_bandwidth, protocol_counts_df, ip_communication_table, protocol_frequency,
    ...
    logger.info(
        tabulate(ip_communication_protocols, headers=["Source IP", "Destination IP", "Protocol", "Count", "Percentage"],
                 tablefmt="grid", floatfmt=".2f"))
...
def main(pcap_file, port_scan_threshold):
    ...
    total_bandwidth, protocol_counts, ip_communication_table, protocol_frequency, ip_communication_protocols = analyze_packet_data(
        df)
